...
正如在第 5.3 节中将证明的，在训练点估计网络时，该方法等同于权重为 $1/σ$ 的加权 ℓ2 正则化。概率编程语言 `stan` 提供了一些在知道所考虑参数预期规模的情况下，如何选择 $σ$ 的例子 [21]。

但是，虽然该方法在实践中被普遍使用，但并没有理论上的依据表明其优于任何其他表示 [80]。正态法因其数学特性和其对数的简单表示而受到青睐，因为大多数学习算法中都使用了概率分布的对数。
...
使用方程 25 中的表述，在某些情况下可以使用函数模型的预期行为来扩展先验。为此，人们通常定义一个一致性条件 C(θ,x) 来评估输入 x 和参数集θ下的预测的相对对数可能性。例如，C 可以被设置为有利于稀疏或有规律的预测，鼓励预测与某些输入变量的同调性（例如，得流感的概率随着年龄的增长而增加），或者在进行半监督学习时有利于低密度区域的决策边界（第 6 节）。

$$
C(\theta)=\int_{\boldsymbol{x}} C(\theta, x) p(x) d x
$$

 (26) 在实践中，$p(x)$ 是未知的，$C(\theta)$ 是从训练集的特征中近似出来的： 
 
$$
C(\theta) \approx \frac{1}{\left|D_{x}\right|} \sum_{\boldsymbol{x} \in\left|D_{x}\right|} C(\theta, x)
$$
 
 现在我们可以写一个与包含一致性条件的先验成比例的函数：
...
$$
 ...
 其中 $p(θ)$ 是没有一致性条件的先验。 监督的程度和先验知识的替代形式到目前为止介绍的架构主要集中在贝叶斯神经网络在监督学习环境中的使用。然而，在现实世界的应用中，获得地面真实标签可能是昂贵的，因此应该采用新的学习策略 [72]。我们现在介绍如何使贝叶斯神经网络适应不同程度的监督。在这样做的同时，我们还展示了贝叶斯后验的表述，它来自于下面介绍的不同的贝叶斯后验（图 8、9 和 10），可以用来（第 5.3 节）获得一个合适的最大后验估计器的损失函数，在这种情况下，一个点估计神经网络足以满足所考虑的使用情况。
