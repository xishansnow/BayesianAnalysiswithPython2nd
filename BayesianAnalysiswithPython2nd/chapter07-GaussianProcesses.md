...
这类似于第 3 章“线性回归模型”中做出的假设。此处的主要区别在于，我们将在 $f$ 上设置先验以体现相邻 $y$ 之间的某些依赖（相关）关系，而高斯过程正适合做这个工作：

```{math}
f(x) \sim \mathcal{G} \mathcal{P}\left(\mu_{x}, K\left(x, x^{\prime}\right)\right) \tag{式7.6} \label{式7.6}
```

$\mathcal{GP}$ 表示高斯过程分布， $\mu_x$ 为均值函数， $K(x,x')$ 为核（或协方差）函数。注意我们使用 `函数` 这个词汇，表示在数学上均值和协方差都是无限的对象，尽管实践中总是作为有限对象来处理的。

如果先验是高斯过程分布，似然是正态分布，那么后验也应当是高斯过程分布，根据共轭先验的知识可以解析地计算出来：
...
$X$ 是观察到的数据点，$X_*$ 表示测试点；也就是我们希望推断得出函数值的新点。

像往常一样，PyMC3 会自动完成整个推断过程，高斯过程也不例外。让我们继续创建一些数据，然后构建一个 PyMC3 模型：
...
在图 7.4 中，我们看到真实的未知函数是一条黑色的虚线，而点则表示了未知函数的带噪声样本。

请注意，为了将公式 7.7 和 7.8 编码到 PyMC3 模型中，需要设置指数族二次核的 $\epsilon$ 参数、核宽 $\ell$ 和正态似然方差。
