...
另外一种方式是绘制被预测变量值 $\hat y$ 的 `HPD`（例如 94%和 50%）区间（见`图 3.7`），用于显示模型预测中 94% 和 50%的数据分布范围。在图中将 `50% HPD 区间` 用深灰色区域表示，将 `94% HPD` 区间用浅灰色表示。
...
# 绘制被预测变量的不确定性区间（指定的 0.5 和默认的 0.94）         
...
计算皮尔逊相关系数的另一种方法是估计多变量高斯分布的协方差矩阵。多元高斯分布是高斯分布在一维以上的推广。以二维为例，要完全描述一个二元高斯分布，需要两个均值（或一个具有两个元素的向量），每个高斯一个。我们还需要一个 2x2 的协方差矩阵，如下所示：
...
这里 $\Sigma$ 为希腊大写的 sigma 字母，通常使用它表示协方差矩阵。在主对角线上，有每个变量的方差，用其标准差 $\sigma_{\mathbb{x}_1}、\sigma_{\mathbb{x}_2}$  的平方来表示。矩阵中其余元素是协方差（变量之间的方差），用单个标准差和变量间的皮尔逊相关系数 $\rho$ 表示。请注意，我们只有一个 $\rho$ ，因为只有两个维度。对于三个变量，我们会有三个皮尔逊相关系数。
...
根据生成数据的方式，可以看出这些变量已经是中心化了。因此，不需要再进一步对数据进行中心化处理来加速推断过程了。事实上该例子中的数据已经是归一化的了。
