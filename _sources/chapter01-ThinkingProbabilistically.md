...
（1）给定一些数据以及关于这些数据是如何生成的一些假设，通过组合概率分布的构建块来设计一个模型。这些模型都是粗略近似的，但大多数情况下满足需要。
...
贝叶斯模型是基于概率构建的，因此也称作概率模型。为什么基于概率呢？因为概率能够很好地描述数据中的不确定性，并让我们在满是岔路的花园里面轻松散步。
...
$$
上式的读法是：$A$ 和 $B$ 同时发生的概率等于 $B$ 发生的概率乘以在 $B$ 发生条件下 $A$ 也发生的概率。其中，$p(A,B)$ 表示 $A$ 和 $B$ 的联合概率，指 $A$ 和 $B$ 同时发生的概率； $p(A|B)$ 表示条件概率 ，指在知识（或证据、事件） $B$ 支持下，$A$ 发生的概率。二者的现实意义不同，例如：“路面是湿的” 概率跟 “（给定条件）下雨时路面是湿的” 概率截然不同。
