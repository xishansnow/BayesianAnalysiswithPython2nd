...
原理上，贝叶斯模型平均 ( BMA ) 是非常自然的，因为用于数据分析的贝叶斯框架可以很容易地结合参数不确定性和模型不确定性 [73, 87]。贝叶斯模型平均不是计算单个模型参数的后验分布，而是计算不同模型后验的加权组合。
...
假设向量 $y=\left(y_{1}, \ldots, y_{n}\right)^{\top}$ 包含一组结果变量的值，我们希望对 $M$ 个嵌套的模型求平均值，其中 $\beta_{m}$ 是第 $m$  个模型中对应的参数向量。正如符号所暗示的那样，$\beta_{m}$ 通常是一组回归系数，但也可能是其他类型的参数，例如在正态线性模型中误差方差。假设我们现在对估计一个标量类型的参数 $\theta$ 感兴趣，则  $\theta$ 的模型平均后验由下式给出：

$$
p(\theta \mid y)=\sum_{m=1}^{M} p(m \mid y) p(\theta \mid y, m) \tag{2.1}
$$

其中 $p(m \mid y)$ 是模型  $m$ 为真的后验概率，  $p(\theta \mid y, m)$ 是当我们假设模型 $m$ 为真时，参数  $\theta$ 的后验。因此， $p(\theta \mid y)$ 是不同模型后验分布的加权组合，权值为`模型后验概率` $p(m \mid y)$ 。使用贝叶斯定理，模型 $m$ 的后验概率为：

$$
p(m \mid y) \propto p(m) p(y \mid m) \tag{2.2}
$$

其中， $p(m)$ 和 $p(y \mid m)$ 分别为模型 $m$ 的先验概率和边缘似然。

根据单模型的贝叶斯定理，边缘似然为该模型参数空间上的积分项：
...
是比较模型  $m$ 和模型 $1$得到的贝叶斯因子 ，模型 $1$ 可以是任意一个参考模型 `[114,146]`。众所周知，贝叶斯因子对参数的先验分布非常敏感， 即便 $n$ 很大时也一样 `[8,97]`。当一个或多个先验不合适时会导致的极端情况，就是式（2.3）中的边缘似然无法被很好的定义`[73, 161]`。

**（3）参数的模型平均后验：常用统计量**

关于 $\theta$ 模型平均后验的统计量是其`均值`和`方差`，由下式定义：
...
其中 $\mathrm{E}(\theta \mid y, m)$ 和 $\operatorname{var}(\theta \mid y, m)$  分别是模型 $m$ 的参数 $\theta$  的后验分布的均值和方差。因此，模型平均后验的方差会受到与每个模型有关的参数不确定性影响，也会受到后验均值在模型间的变化性影响。

我们可以使用模型平均后验去计算 $\theta$ 的 $100(1-2 \alpha) \%$ 的中心可信区间，用  $\left[\theta_{L}, \theta_{U}\right]$ 来表示：

$$
\int_{-\infty}^{\theta_{L}} p(\theta \mid y) d \theta=\int_{\theta_{U}}^{\infty} p(\theta \mid y) d \theta=\alpha  \tag{2.6}
$$

此外，也可以使用最高后验密度可信区间 $100(1-2 \alpha) \%$ 。不过，中心可信区间相较最高密度区间计算更为容易有些，并且其极限可以被解释为后验的中位数。在本文实例中，我们均使用中心可信区间。

经典贝叶斯模型平均的一个副产品是可以计算每个预测变量的后验包含概率 （ Posterior Inclusion Probability, PIP ），即所有包含该预测变量的模型后验概率之和  `[7,12,36]` 。这提供了对每个预测变量的相对重要性的有用总结。然而，后验包含概率会受到模型集选择的影响，尤其是预测变量的重要性可能会由于参与了更多模型而被夸大 [66]。此外，通过比较一组预测变量值对应的结果变量期望值的模型平均后验分布，可以获得更有用的相对重要性总结（第 1.4 节）。在频率主义的模型平均（第 3.7 节）中使用加权求和模型会出现类似的效果。
